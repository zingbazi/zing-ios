...
{
	NSString *permissionsString = [FacebookManager JSONStringFromObject:[[FacebookManager sharedManager] sessionPermissions]];
...
void _facebookShowDialog( const char * dialogType, const char * json )
{
	...
	if( jsonString && [jsonString isKindOfClass:[NSString class]] && jsonString.length )
		dict = [[FacebookManager objectFromJsonString:jsonString] mutableCopy];
...
void _facebookGraphRequest( const char * graphPath, const char * httpMethod, const char * jsonDict )
{
	...
	NSDictionary *dict = (NSDictionary*)[FacebookManager objectFromJsonString:jsonString];
