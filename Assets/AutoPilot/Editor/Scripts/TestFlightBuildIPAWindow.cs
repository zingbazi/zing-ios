...
class TestFlightBuildIPAWindow : TestFlightMessageWindow
{
	new public static void CreateWindow()
	{
		...
		window.position = new Rect(10, 10, 600, 375);
		window.minSize  = new Vector2(600, 375); 
		window.maxSize  = new Vector2(1024, 375); 
	}	
	...
	{		
		...
		TestFlightPreferencesWindow.DrawSubUserSettingsForIPA(preferences, true, allProvisions, ref allSchemas, allIdentities);
		GUILayout.Space(5);
		GUILayout.EndVertical();
