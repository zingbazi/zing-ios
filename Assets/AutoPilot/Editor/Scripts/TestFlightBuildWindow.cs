...
class TestFlightBuildWindow : TestFlightWindow
{
	...
	private bool IsArchiveBuild 
	{
		get 
		{ 
			return preferences.userPrefs.testFlightSDKEmbedType == TestFlightSDKEmbedType.ForAppStoreSubmission 
	...
	private void NextStage()
	{
		...
		case BuildStage.Upload:
			if(buildIPAOnly)
			{
				string message = "Build IPA complete";
				if(IsArchiveBuild)
					message += "\nBuild has been archived in XCode organizer";
						
				EditorUtility.DisplayDialog("AutoPilot", message, "Ok");
		...
		case BuildStage.Max:
			...
			{
				var message = "Build and upload complete";
				if(IsArchiveBuild)
					message += "\nBuild has been archived in XCode organizer";
