...
[ExecuteInEditMode]
[AddComponentMenu("NGUI/Interaction/SysFont Popup List")]
public class UISysFontPopupList : MonoBehaviour
{
	/// <summary>
	/// Current popup list. Only available during the OnSelectionChange event callback.
	/// </summary>

	static public UISysFontPopupList current;
	...
	public UISysFontLabel textLabel;
	...
	UISprite mHighlight;
	UISysFontLabel mHighlightedLabel = null;
	List<UISysFontLabel> mLabelList = new List<UISysFontLabel>();
	float mBgBorder = 0f;
	...
	void Highlight (UISysFontLabel lbl, bool instant)
	{
		if (mHighlight != null)
		{
			...
			UISysFontLabel previousHighlight = mHighlightedLabel;
			...
			if (instant || !isAnimated)
			{
				...
				mHighlightedLabel.color = Color.white;
				if(previousHighlight != null) previousHighlight.color = textColor;
			}
			else
			{
				TweenPosition.Begin(mHighlight.gameObject, 0.1f, pos).method = UITweener.Method.EaseOut;
				TweenColor.Begin(mHighlightedLabel.gameObject, 0.1f, Color.white);
				if(previousHighlight != null) TweenColor.Begin(previousHighlight.gameObject, 0.1f, textColor);
			}
		}
	}
	...
	void OnItemHover (GameObject go, bool isOver)
	{
		if (isOver)
		{
			UISysFontLabel lbl = go.GetComponent<UISysFontLabel>();
			Highlight(lbl, false);
		}
	}
	...
	void Select (UISysFontLabel lbl, bool instant)
	{
	...
	void OnItemPress (GameObject go, bool isPressed) { if (isPressed) Highlight(go.GetComponent<UISysFontLabel>(), false); }
	void OnItemClick (GameObject go) { Select(go.GetComponent<UISysFontLabel>(), false); }
	void OnItemSelect (GameObject go, bool selected) { if(selected) Select(go.GetComponent<UISysFontLabel>(), false); }
	...
	void OnSelect (bool isSelected)
	{
		if(mChild != null)
		{
			Bounds popupBounds = NGUIMath.CalculateAbsoluteWidgetBounds(mChild.transform);
//			Vector3 min = popupBounds.min;
//			Vector3 max = popupBounds.max;
//			max.z = 10f;
//			min.z = -10f;
//			popupBounds.SetMinMax(min, max);
			Vector2 pos = UICamera.currentTouch.pos;
			
			Vector3 worldPos = UICamera.FindCameraForLayer(mChild.layer).camera.ScreenToWorldPoint(new Vector3(pos.x, pos.y));
			
//			bool outside = !popupBounds.Contains(worldPos);
			
			bool outside = (worldPos.x < popupBounds.min.x || worldPos.x > popupBounds.max.x) && (worldPos.y < popupBounds.min.y || worldPos.y > popupBounds.max.y);
	...
	void AnimateScale (UIWidget widget, bool placeAbove, float bottom)
	{
		...
		float minSize = fontSize * textScale + mBgBorder * 2f;
	...
	{
		if (mChild == null && atlas != null && cocoaFontName != null && androidFontName != null && items.Count > 0)
		{
			mLabelList.Clear();
...
//			float fontScale = font.size * font.pixelSize * textScale;
			float fontScale = fontSize * textScale;
			float x = 0f;
			float y = 0f;
			List<UISysFontLabel> labels = new List<UISysFontLabel>();
			...
			for (int i = 0, imax = items.Count; i < imax; ++i)
			{
				...
				UISysFontLabel lbl = NGUITools.AddWidget<UISysFontLabel>(mChild);
				lbl.pivot = UIWidget.Pivot.TopLeft;
				lbl.AppleFontName = cocoaFontName;
				lbl.AndroidFontName = androidFontName;
				lbl.FontSize = fontSize;
				lbl.Text = (isLocalized && Localization.instance != null) ? Localization.instance.Get(s) : s;
				lbl.color = textColor;
				lbl.UpdateTextureIfNecessary();
				lbl.MakePixelPerfect();	
				lbl.cachedTransform.localPosition = new Vector3(bgPadding.x + padding.x, y - padding.y, -0.01f);
				...
				y -= fontScale;
				y -= padding.y * 2f;
				x = Mathf.Max(x, lbl.cachedTransform.localScale.x);
	...
	public delegate bool PopupWillBeDisplayed(UISysFontPopupList popup);
	public event PopupWillBeDisplayed PopupWillBeDisplayedDelegate;
	
	void OnClick()
	{
		if(PopupWillBeDisplayedDelegate != null) {
			if(!PopupWillBeDisplayedDelegate(this)) {
				return;
			}
		}
		DisplayPopup();
